...
class JetsonPiConfig:
    """In-process Jetson-PI-Edge PI0/PI0.5 configuration."""
...
def _make_jetson_pi_processors(
    ...
    """Load only the checkpoint's state normalizer and action unnormalizer."""
...
ROBOT_STATE_DIM = 6
SERVER_STATE_DIM = 32
SERVER_ACTION_STEPS = 50
SERVER_ACTION_DIM = 32
ROBOT_ACTION_DIM = 6


def _prepare_jetson_image(image: Any) -> np.ndarray:
    """Apply PI0.5 resize-with-pad and return one contiguous RGB uint8 image."""
...
def _prepare_jetson_state(state: Any) -> np.ndarray:
    """Pad one normalized SO-101 state from 6 to 32 dimensions."""
    ...
    if state.shape != (ROBOT_STATE_DIM,):
        raise ValueError(f"expected state shape ({ROBOT_STATE_DIM},), got {state.shape}")
    if not np.isfinite(state).all():
        raise ValueError("state contains non-finite values")
    return np.pad(state, (0, SERVER_STATE_DIM - ROBOT_STATE_DIM)).astype(np.float32, copy=False)


class JetsonPiChunkPolicy:
    """Run Jetson-PI-Edge in process and expose LeRobot's policy interface."""

    def __init__(self, cfg: JetsonPiConfig):
        if not 0 < cfg.chunk_size <= SERVER_ACTION_STEPS:
            raise ValueError(f"jetson_pi.chunk_size must be in [1, {SERVER_ACTION_STEPS}]")
        self.cfg = cfg
        self.config = SimpleNamespace(device="cpu", use_amp=False)
        self._model = jetson_pi.load_model(
            model_path=cfg.model_path,
            mmproj_path=cfg.mmproj_path,
            backend=cfg.backend,
            n_views=len(cfg.image_keys),
    ...
    def _request_actions(self, observation) -> None:
        images = np.ascontiguousarray(
            np.stack(
                [
                    _prepare_jetson_image(observation[f"observation.images.{key}"])
                    for key in self.cfg.image_keys
                ]
            ),
            dtype=np.uint8,
        )
        state = _prepare_jetson_state(observation["observation.state"])
        ...
        expected = (SERVER_ACTION_STEPS, SERVER_ACTION_DIM)
        if actions.shape != expected:
            raise RuntimeError(f"expected action shape {expected}, got {actions.shape}")
        if not np.isfinite(actions).all():
            raise RuntimeError("Jetson-PI returned non-finite actions")
        self._action_queue.extend(actions[: self.cfg.chunk_size, :ROBOT_ACTION_DIM].tolist())
...
class RecordConfig:
    ...
    # Whether to run a Jetson-PI-Edge PI0/PI0.5 model in process.
    ...
    def __post_init__(self):
        ...
        if self.teleop is None and self.policy is None and self.jetson_pi is None:
            raise ValueError("Choose a policy, a jetson_pi model, a teleoperator, or a combination")
        if self.policy is not None and self.jetson_pi is not None:
            raise ValueError("Choose either policy or jetson_pi, not both")
...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    try:
        ...
        # Load either the standard LeRobot policy or the in-process Jetson-PI model.
        preprocessor = None
        postprocessor = None
        ...
        if isinstance(policy, JetsonPiChunkPolicy):
            policy.close()
